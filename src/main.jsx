...
function Admin({ anime, setAnime, openAnime, users, setUsers, applications, setApplications }) {
  ...
  const importAnime = (item) => {
    setAnime((current) => {
      const existing = current.find((candidate) => candidate.sourceUrl === item.sourceUrl || candidate.id === item.id);
      return existing
        ? current.map((candidate) => candidate.id === existing.id ? { ...candidate, ...item, id: candidate.id } : candidate)
        : [item, ...current];
    });
    setShowAnimeImport(false);
    notify(`${item.name} importé avec ses saisons, épisodes et lecteurs.`);
  };
